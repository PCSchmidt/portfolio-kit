# Family status — gate-enforced-rag Phase 4 2026-09-03

**Contract version:** 0.1.0  
**Capacity:** 8–12 hours/week  
**This session:** gate-enforced-rag Phase 4 (local JSON observability)  
**Previous:** honesty-gate Phases 1–4; bake-off Phases 1–4; living-docs Phases 1–5; meridian-jspace Phase 1; gate-enforced-rag Phases 1–3
...
### gate-enforced-rag (Phase 4)

Mechanical RAG (`src/answer.js`) with CI-safe adapters (`src/adapters.js`), a local multi-silo router (`src/router.js`), and local JSON traces (`src/observe.js`). Keyword retrieve + extractive citations, Evaluator gate before delivery. Unresolved cross-silo contradictions fail closed. GitHub writes, live LLM, live Haystack / LlamaIndex packages, Wikipedia / arXiv federation, and OTEL export refused. No GPU, no embeddings API in CI. Eval GER-001–028.
...
gate-enforced-rag Phase 4 shipped. Next: meridian-jspace Phase 2 (optional live Qwen) or RAG Phase 5 unless redirected.
...
3. **Bake-off Phase 4 shipped.** Next implementation in this family is meridian-jspace Phase 2 or gate-enforced-rag Phase 5 — not red/blue.
...
5. **Later family:** meridian-jspace Phase 2+ → gate-enforced-rag Phase 5 → redteam-blue-gate.
...
3. Do not start red/blue. Next: meridian-jspace Phase 2 or gate-enforced-rag Phase 5 unless the user redirects. Do not download Qwen unless asked for meridian-jspace Phase 2.
